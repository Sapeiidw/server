            ...
            <div class="mx-auto px-4 sm:px-8 py-8">
                ...
                <form action="{{ route('client.update', $client->id) }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="mt-4">
                        ...
                        <x-jet-input class="block w-full mt-1" type="text" name="link" value="{{ $client->link }}"/>
                        @error('link')
                            <span class="text-red-900 p-2">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="mt-4">
                        <x-jet-label for="photo" value="Name" />
                        <x-jet-input class="block w-full mt-1" type="file" name="photo"/>
                        @error('photo')
                            <span class="text-red-900 p-2">{{ $message }}</span>
                        @enderror
                    </div>
